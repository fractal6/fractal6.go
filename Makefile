...
run_api:
	#go run main.go api
	while true; do $(GO_WATCH) | entr -rcc go run main.go api; sleep 0.5; done


run_notifier:
	#go run main.go notifier
	while true; do $(GO_WATCH) | entr -rcc go run main.go notifier; sleep 0.5; done
...
test-vet:
	go vet ./...
	go vet -tags integration ./db/... ./web/auth/... ./web/handlers/...
