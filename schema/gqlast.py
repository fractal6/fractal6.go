...
class SemanticFilter:
    ...
    def get_fields(ast):
        ...
        * remove comments
        ...
        if not ast._fields or len(ast._fields) != 3:
            raise ValueError(
                """Parsing error: field not found. Please check your grammar and be cautious with multiline comments.
                         AST: %s"""
                % ast
            )
    ...
    def inherit_interface(self, ast):
        ...
        for itf_fd in self.interfaces[interface_name]:
            ...
            self._ast_set(
                curfd, "_directives", [x["ast"] for x in directives if x["name"] not in cur_directives]
            )
            # LOG DEBUG
    ...
    def copy_directives(
        ...
        if not _fields:
            logging.warning("Type `%s` unknown" % name_in)
            return
            ...
            for f in data_out[name_out]:
                ...
                if type_ in data_in:
                    for directive_ in data_in[type_ + "__directives"]:
                        ...
                        self._ast_set(
                            pre_directive, "_name", pre_directive_name
                        )  # @warning: breaks the original Grammar syntax.
                        ...
                        for i, a in enumerate(args):
                            if (
                                isinstance(a, dict)
                                and hasattr(a, "field")
                                and self.get_name(a.field) in _input_names
                            ):
                        ...
                        if pre_directive_name not in self.extra_directives:
                            directive_definition = "directive @%s on ARGUMENT_DEFINITION" % (
                                pre_directive_name
                            )
                        ...
                        if op in ("add", "update", "delete"):
                            ...
                            self._ast_set(
                                post_directive, "_name", post_directive_name
                            )  # @warning: breaks the original Grammar syntax.
                            ...
                            if post_directive_name not in self.extra_directives:
                                directive_definition = "directive @%s on FIELD_DEFINITION" % (
                                    post_directive_name
                                )
                                self.extra_directives.append(directive_definition)
        ...
        * add filtered directive
        ...
        if name.startswith("Add") and name.endswith("Input"):
            ...
            if type_name:
                ...
                self.sf.copy_directives(
                    type_name, ["types", "interfaces"], name, "inputs", r"^x_(add|alter)", with_args=True
                )
            ...
            if type_name and not type_name in self.sf.unions:
                ...
                self.sf.copy_directives(
                    type_name, ["types", "interfaces"], name, "inputs", r"^x_", remove_x_alter=True
                )
...
class SDL:
    ...
    def __init__(self, settings):
        ...
        self.ast = self.parser.parse(
            self._target, rule_name="start", semantics=self.semantics, parseinfo=False
        )
    ...
    def stringify(self, ast=None, out=None, root=False, _prev=None, _next=None, ignore_nl=False):
        ...
        if ast is None:
            root = True
            ast = [list(map(lambda x: x + "\n", self.sf.extra_directives)), self.ast]
            out = [nl]
        ...
        for nth, o in enumerate(ast):
            if isinstance(o, dict):  # AST like
                ...
                for ith, k in enumerate(keys):
                    ...
                    out = self.stringify(
                        [v],  # removing list breaks the space logics
                        out,
                        _prev=_prev,
                        _next=_next,
                        ignore_nl=ignore_nl,
                ...
                for mth, oo in enumerate(o):
                    ...
                    out = self.stringify(
                        [oo],  # removing list breaks the space logics
                        out,
                        _prev=_prev,
                        _next=_next,
                        ignore_nl=ignore_nl,
