...
//
// For tension deletes the template also collects File.storageKey of every
// attachment carried by the tension's (and contract's) comments. The Dgraph
// rows are dropped as part of the cascade; the matching S3 objects are fired
// to a goroutine so an empty bucket policy or 5xx doesn't block the txn.
